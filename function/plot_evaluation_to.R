...
plot_evaluation = function(folder,out){
  ...
  get_res = function(model,time,v,tp,k){
    ...
    sd = sd(dt[,as.character(time)],na.rm = TRUE)
    se = sd/sqrt(n)
  ...
  df2 = subset(out,model2 %in% c('JM_cumulative','JM_value','JM_value-slope'))  %>%
    melt(.,measure.vars = c('12','15'),variable.name = 'time') %>%
    subset(.,time %in% c('12','15'))%>% rename( evaluation = variable  )
  ...
  df = subset(out,model2 %in% c('JM_cumulative','LM_P2','LM_bi'))  %>%subset(.,time %in% c('12','15')) %>%
    melt(.,measure.vars = c('12','15'),variable.name = 'time') %>% rename( evaluation = variable  ) 
